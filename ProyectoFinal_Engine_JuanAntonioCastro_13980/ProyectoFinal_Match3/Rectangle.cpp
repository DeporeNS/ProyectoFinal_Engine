...
// Constructor
Rectangle::Rectangle(int index) : Primitives(index) {
	mTexture[0] = new Texture("Tile2.png");
	mTexture[0]->Parent(this);
	mTexture[0]->Pos(VEC2_ZERO);

	mTexture[1] = new Texture("Tile2Selected.png");
	mTexture[1]->Parent(this);
	mTexture[1]->Pos(VEC2_ZERO);

	mTexture[2] = new Texture("Tile2Over.png");
